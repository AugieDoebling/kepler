...
def new_message(content: str, role: str ='user'):
      return {
         'role': role,
         'content': content
      }
...
messages = background_content_messages

while True:

   response: ChatResponse = chat(model='gemma4:e4b', messages=messages)
   ...
   #       # messages.append(new_message(call_result, "tool"))

   #    # for tool calls, keep calling until you get a chat response
   #    continue

   # else:
   messages.append(new_message(response.message.content, "agent"))

   print(response.message.content)

   user_message = input("\nUser: ")
   messages.append(new_message(user_message))
